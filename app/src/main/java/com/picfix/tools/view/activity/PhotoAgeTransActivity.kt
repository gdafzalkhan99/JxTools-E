...
class PhotoAgeTransActivity : BaseActivity() {
    ...
    override fun setLayout(): Int {
        return R.layout.a_photo_age_trans
    }
    ...
    private fun choosePic(index: Int) {
        when (index) {
            0 -> Glide.with(this).load(R.drawable.iv_age_trans_after_1).into(bigPic)
            1 -> Glide.with(this).load(R.drawable.iv_age_trans_after_2).into(bigPic)
        }
    }
    ...
    private fun toImagePage(uri: Uri) {
        ...
        intent.putExtra("from", "age_trans")
