...
import com.picfix.tools.callback.DocCallback
...
class DocRecoveryActivity : BaseActivity() {
    ...
    override fun initData() {
        ...
        searchDocs()
        LogReportManager.logReport("文档恢复页", "访问页面", LogReportManager.LogType.OPERATION)
    }
        ...
        val callback = object : DocCallback {
            ...
            override fun onFailed(step: Enum<FileStatus>, message: String) {
                launch(Dispatchers.Main) {
                    ...
                    ToastUtil.showShort(this@DocRecoveryActivity, message)
                }
            }
        }

        thread {
            WxManager.getInstance(this@DocRecoveryActivity).getWxDocs(this, callback)
        }
    }
    ...
    private fun sortAllSelectedList() {
        ...
        launch(Dispatchers.IO) {
            sortDocs = DBManager.getDocByKey(this@DocRecoveryActivity, type, minSize, maxSize, minDate, maxDate)
            launch(Dispatchers.Main) {
                mainDocs.clear()
                if (sortDocs.size > 0) {
    ...
    private fun deleteDocs() {
        ...
        if (pay) {
            launch(Dispatchers.IO) {
                ...
                WxManager.getInstance(this@DocRecoveryActivity).deleteFile(deleteList, object : FileWithTypeCallback {
                    override fun onSuccess(step: Enum<FileStatus>) {
                        launch(Dispatchers.Main) {
                        }
                    }
