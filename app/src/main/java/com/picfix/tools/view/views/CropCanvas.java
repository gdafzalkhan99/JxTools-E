 ...
 */
public class CropCanvas extends AppCompatImageView {
    ...
    public void imageScale() {
        ...
        ChooseArea = new RectF(dst.left + 100, dst.top + 100, dst.right - 100, dst.bottom - 100);
        this.setPressRecLoc();
        this.invalidate();
    }
    ...
    public Bitmap getSubsetBitmap() {
        float ratioWidth = bitMap.getWidth() / (dst.right - dst.left);
        float ratioHeight = bitMap.getHeight() / (dst.bottom - dst.top);
     ...
     * @return 区域信息
     */
    public HashMap<String, Object> getSelectAreaMap() {
        float ratioWidth = bitMap.getWidth() / (dst.right - dst.left);
        float ratioHeight = bitMap.getHeight() / (dst.bottom - dst.top);
        int left = (int) ((ChooseArea.left - dst.left) * ratioWidth);
        int right = (int) (left + (ChooseArea.right - ChooseArea.left) * ratioWidth);
...
//        set_LeftArea_Alpha();
        ...
        JLog.i("width = " + width);
        JLog.i("height = " + height);
        JLog.i("left = " + left);
        JLog.i("top = " + top);

        HashMap<String, Object> rectangleMap = new HashMap<>();
        rectangleMap.put("width", width);
        rectangleMap.put("top", top);
        rectangleMap.put("height", height);
        rectangleMap.put("left", left);
    ...
    private void setPressRecLoc() {
        recLT.set(ChooseArea.left - 20, ChooseArea.top - 20, ChooseArea.left + 20, ChooseArea.top + 20);
        recLB.set(ChooseArea.left - 20, ChooseArea.bottom - 20, ChooseArea.left + 20, ChooseArea.bottom + 20);
        recRT.set(ChooseArea.right - 20, ChooseArea.top - 20, ChooseArea.right + 20, ChooseArea.top + 20);
        recRB.set(ChooseArea.right - 20, ChooseArea.bottom - 20, ChooseArea.right + 20, ChooseArea.bottom + 20);
    }
