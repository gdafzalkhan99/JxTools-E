...
import com.picfix.tools.callback.VoiceCallback
...
class VoiceRecoveryActivity : BaseActivity() {
    ...
    override fun initData() {
        ...
        searchVoices()
        LogReportManager.logReport("语音恢复页", "访问页面", LogReportManager.LogType.OPERATION)
    }
...
//                }
    ...
    private fun sortAllSelectedList() {
        ...
        launch(Dispatchers.IO) {
            sortVoices = DBManager.getVoiceByKey(this@VoiceRecoveryActivity, type, minSize, maxSize, minDate, maxDate)
        ...
        val callback = object : VoiceCallback {
            ...
            override fun onFailed(step: Enum<FileStatus>, message: String) {
                launch(Dispatchers.Main) {
                    ...
                    ToastUtil.showShort(this@VoiceRecoveryActivity, message)
                }
            }
        }

        thread { WxManager.getInstance(this@VoiceRecoveryActivity).getWxVoices(this, callback) }
    }
    ...
    private fun deletePics() {
        ...
        if (pay) {
            launch(Dispatchers.IO) {
                ...
                WxManager.getInstance(this@VoiceRecoveryActivity).deleteFile(deleteList, object : FileWithTypeCallback {
                    override fun onSuccess(step: Enum<FileStatus>) {
                        launch(Dispatchers.Main) {
                        }
                    }
