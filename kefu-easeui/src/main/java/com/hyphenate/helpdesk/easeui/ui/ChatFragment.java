...
import com.hyphenate.helpdesk.easeui.util.Dict;
...
public class ChatFragment extends BaseFragment implements ChatManager.MessageListener, EmojiconManager.EmojiconManagerDelegate {
    ...
    protected int[] itemdrawables = {R.drawable.hd_chat_takepic_selector, R.drawable.hd_chat_image_selector, R.drawable.hd_chat_video_selector,
    ...
    protected void initView() {
        // 消息列表layout
        messageList = Objects.requireNonNull(getView()).findViewById(R.id.message_list);
        messageList.setShowUserNick(showUserNick);
        //设置机器人
        messageList.robotShow = robotShow;
        messageList.isPay = isPay;

        listView = messageList.getListView();
        tvTipWaitCount = getView().findViewById(R.id.tv_tip_waitcount);
        ...
        swipeRefreshLayout.setColorSchemeResources(R.color.colorAccent, R.color.holo_green_light,
    ...
    protected void setListItemClickListener() {
        messageList.setItemClickListener(new MessageListItemClickListener() {
            ...
            public void onResendClick(final Message message) {
                new AlertDialog(getActivity(), R.string.resend, R.string.confirm_resend, null, (confirmed, bundle) -> {
                    if (!confirmed) {
                        return;
                    }
...
//            messageList.refresh();
        ...
        public void onExtendMenuItemClick(int itemId, View view) {
            ...
            switch (itemId) {
                case ITEM_TAKE_PICTURE: // 拍照
                    PermissionsManager.getInstance().requestPermissionsIfNecessaryForResult(ChatFragment.this,
    ...
    protected void emptyHistory() {
        ...
        new AlertDialog(getActivity(), null, msg, null, (confirmed, bundle) -> {
            if (confirmed) {
    ...
    protected void sendTextMessage(String content) {
        ...
        messageList.addNewMessage(content, message);
    }
    ...
    protected void sendImageMessage(String imagePath) {
        ...
        Message message = Message.createImageSendMessage(imagePath, true, toChatUsername);
        attachMessageAttrs(message);
        ChatClient.getInstance().chatManager().sendMessage(message);
    ...
    protected void sendImageMessage(Uri imageUri) {
        Message message = Message.createImageSendMessage(imageUri, true, toChatUsername);
        if (message != null) {
            attachMessageAttrs(message);
            ChatClient.getInstance().chatManager().sendMessage(message);
            messageList.refreshSelectLastDelay(MessageList.defaultDelay);
